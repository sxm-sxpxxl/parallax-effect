...
public class ParallaxLayerPropertyDrawer : PropertyDrawer
{
    ...
    {
        return CountProperties * EditorGUIUtility.singleLineHeight * (EditorGUIUtility.wideMode ? 1 : 2) + 2f * Padding + Offset;
    }
    ...
    public override void OnGUI(Rect rect, SerializedProperty property, GUIContent label)
    {
        EditorGUI.BeginProperty(rect, label, property);
        {
            ...
            Rect firstPropertyRect = new Rect(new Vector2(rect.position.x, rect.position.y + Padding), new Vector2(rect.size.x, propertyHeight));
            Rect secondPropertyRect = new Rect(new Vector2(rect.position.x, rect.position.y + Padding + propertyHeight + Offset), new Vector2(rect.size.x, propertyHeight));
